      ...
      {showIntro ? (
        <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-gradient-to-br from-slate-950 via-slate-900 to-slate-800 px-6 text-white">
          <motion.div
            ...
            className="absolute inset-0 bg-[radial-gradient(circle_at_center,white_0%,transparent_55%)]"
          />
          <motion.div
          ...
          >
            <motion.div
              ...
              className="mb-4 inline-flex items-center rounded-full border border-white/20 bg-white/10 px-4 py-2 text-sm tracking-[0.2em] text-white/80 backdrop-blur"
            >
              SENS MATH · HIGH SCHOOL FIT ANALYZER
            </motion.div>
            <motion.h1
              initial={{ opacity: 0, y: 18 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.35, duration: 0.9 }}
              className="whitespace-pre-line text-3xl font-bold leading-tight md:text-6xl"
            >
              {"쎈수학지족학원
중3 고등학교 성향 적합도 진단프로그램"}
            </motion.h1>
            <motion.p
              initial={{ opacity: 0, y: 18 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.55, duration: 0.9 }}
              className="mx-auto mt-6 max-w-2xl text-sm leading-7 text-white/75 md:text-lg"
            >
            ...
            >
              <Button onClick={() => setShowIntro(false)} className="rounded-2xl px-8 py-6 text-base font-semibold">
                진단 시작하기
              </Button>
      ...
      <div className="mx-auto max-w-7xl p-6 md:p-10">
          ...
          <Card className="rounded-3xl border-0 shadow-lg">
            <CardHeader>
              ...
              <CardDescription className="text-base leading-7">
                문항에는 학교 이름이 드러나지 않도록 설계했습니다. 학생·학부모가 50개 항목을 체크하면,
                성향 점수를 바탕으로 5개 학교의 적합도를 1순위부터 5순위까지 자동 정리합니다.
              </CardDescription>
            </CardHeader>
          </Card>
          ...
          <Card className="rounded-3xl border-0 shadow-lg">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                <Badge variant="secondary" className="rounded-full px-3 py-1">예상 유형: {studentTierLabel(answeredAvg)}</Badge>
                <Badge variant="outline" className="rounded-full px-3 py-1">문항 수: 50</Badge>
              </div>
            </CardContent>
          </Card>
        </motion.div>
        ...
        <Tabs defaultValue="questions" className="space-y-6">
          <TabsList className="grid w-full grid-cols-3 rounded-2xl bg-white p-1 shadow-sm">
            <TabsTrigger value="questions" className="rounded-2xl">체크리스트</TabsTrigger>
            <TabsTrigger value="results" className="rounded-2xl">추천 결과</TabsTrigger>
            <TabsTrigger value="method" className="rounded-2xl">활용 방법</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="questions" className="space-y-6">
            {GROUPS.map(([start, end], idx) => {
              ...
              return (
                <Card key={idx} className="rounded-3xl border-0 shadow-md">
                  <CardHeader>
                    ...
                    {items.map((q) => (
                      <div key={q.id} className="rounded-2xl border border-slate-200 p-4">
                        <div className="mb-3 flex items-start gap-3">
                          <Badge className="mt-0.5 rounded-full">{q.id}</Badge>
                          <div>
                        ...
                        <div className="grid grid-cols-5 gap-2">
                          {SCALE.map((s) => (
                            <button
                              ...
                              className={`rounded-2xl border px-3 py-3 text-sm transition ${answers[q.id] === s.value ? "border-slate-900 bg-slate-900 text-white" : "border-slate-200 bg-white hover:bg-slate-50"}`}
                            >
            ...
            <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
              <Card className="rounded-3xl border-0 shadow-md">
                <CardHeader>
                  ...
                  <CardDescription>
                    {studentName ? `${studentName} 학생` : "현재 학생"}의 체크 결과를 바탕으로 한 적합도 순위입니다.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  {ranking.map((school, idx) => (
                    <div key={school.key} className={`rounded-3xl border p-5 ${idx === 0 ? "border-slate-900 bg-slate-900 text-white" : "border-slate-200 bg-white"}`}>
                      ...
                      <div className="mt-3 grid gap-2 md:grid-cols-2">
                        <div className="rounded-2xl bg-emerald-500/10 p-3 text-sm leading-6">
                          <div className="font-semibold">성공 가능성이 높은 학생</div>
                          <div>{school.success.join(" / ")}</div>
                        </div>
                        <div className="rounded-2xl bg-rose-500/10 p-3 text-sm leading-6">
                          <div className="font-semibold">주의가 필요한 학생</div>
                          <div>{school.fail.join(" / ")}</div>
                        </div>
                      </div>
                    </div>
                  ))}
              ...
              <div className="space-y-6">
                  ...
                  <CardContent className="h-[340px]">
                    <ResponsiveContainer width="100%" height="100%">
                      <RadarChart data={radarData}>
                        ...
                        <Radar name="성향" dataKey="value" fillOpacity={0.5} />
                      </RadarChart>
                    </ResponsiveContainer>
                  </CardContent>
                </Card>
                ...
                <Card className="rounded-3xl border-0 shadow-md">
                  <CardHeader>
                    <CardTitle>학교별 적합도 비교</CardTitle>
                  </CardHeader>
                  <CardContent className="h-[320px]">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={barData}>
                        ...
                        <YAxis domain={[0, 100]} />
                        <Tooltip />
                        <Bar dataKey="점수" radius={[10, 10, 0, 0]} />
                      </BarChart>
                    </ResponsiveContainer>
                  </CardContent>
                </Card>
          ...
          <TabsContent value="method" className="space-y-6">
            <div className="grid gap-6 md:grid-cols-2">
              <Card className="rounded-3xl border-0 shadow-md">
                <CardHeader>
                  <CardTitle>상담실에서 쓰는 방법</CardTitle>
                </CardHeader>
            ...
            <Card className="rounded-3xl border-0 shadow-md">
              ...
              <CardContent className="space-y-4 text-sm leading-7 text-slate-700">
                ...
                <div>학교별 점수는 공개된 학교 유형, 규모, 교육과정 안내, 자율형공립고 2.0 안내 공개 여부, 단일 성별 여부 같은 공개 요소를 바탕으로 상대적 가중치를 준 내부 모델입니다.</div>
                <div>따라서 이 앱은 “절대적인 정답”보다 “상담을 시작하기 위한 구조화된 진단 도구”로 쓰는 것이 가장 좋습니다.</div>
                <div className="flex gap-3 pt-2">
                  <Button onClick={resetAll} variant="outline" className="rounded-2xl"><RotateCcw className="mr-2 h-4 w-4" /> 초기화</Button>
                </div>
              </CardContent>
            </Card>
          </TabsContent>
